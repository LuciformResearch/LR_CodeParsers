 ...
 *
 * Implements BaseImportResolver for TypeScript and JavaScript projects.
 *
 * Handles:
...
import type { BaseImportResolver, ImportType, ResolvedImport, TypeScriptConfig } from './types.js';
...
export class TypeScriptImportResolver implements BaseImportResolver {
  private tsConfig: TypeScriptConfig | null = null;
  ...
  isLocalImport(importPath: string): boolean {
    // Built-in modules are not local
  ...
  async resolveImportFull(importPath: string, currentFile: string): Promise<ResolvedImport> {
    ...
    return {
      ...
      packageName: importType === 'package' ? this.extractPackageName(importPath) : undefined,
